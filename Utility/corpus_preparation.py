...
from TrainingInterfaces.Text_to_Spectrogram.AutoAligner.autoaligner_train_loop import (
    train_loop as train_aligner,
)
from TrainingInterfaces.Text_to_Spectrogram.FastSpeech2.FastSpeechDataset import (
    FastSpeechDataset,
)
    ...
    if not os.path.exists(os.path.join(corpus_dir, "fast_train_cache.pt")):
        if fine_tune_aligner:
            ...
            if not os.path.exists(os.path.join(aligner_dir, "aligner.pt")):
                aligner_datapoints = AlignerDataset(
                    transcript_dict,
                    cache_dir=corpus_dir,
                    lang=lang,
