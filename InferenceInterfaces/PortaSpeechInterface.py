...
class PortaSpeechInterface(torch.nn.Module):
    def __init__(
        ...
        try:
            ...
            try:
                ...
                self.phone2mel = PortaSpeech(
                    weights=checkpoint["model"], lang_embs=None
                )  # multi speaker single language
            except RuntimeError:
                self.phone2mel = PortaSpeech(
                    weights=checkpoint["model"], lang_embs=None, utt_embed_dim=None
                )  # single speaker
        with torch.no_grad():
        ...
        ################################
        self.mel2wav = HiFiGANGenerator(path_to_weights=vocoder_model_path).to(
            torch.device(device)
        )
        ...
        self.audio_preprocessor = AudioPreprocessor(
            input_sr=16000, output_sr=16000, cut_silence=True, device=self.device
        )
        self.phone2mel.eval()
        self.mel2wav.eval()
    ...
    def set_utterance_embedding(self, path_to_reference_audio="", embedding=None):
        ...
        if sr != self.audio_preprocessor.sr:
            self.audio_preprocessor = AudioPreprocessor(
                input_sr=sr, output_sr=16000, cut_silence=True, device=self.device
            )
        spec = self.audio_preprocessor.audio_to_mel_spec_tensor(wave).transpose(0, 1)
        spec_len = torch.LongTensor([len(spec)])
        self.default_utterance_embedding = self.style_embedding_function(
    ...
    def set_language(self, lang_id):
        ...
        self.set_language_embedding(lang_id=lang_id)
    ...
    def set_language_embedding(self, lang_id):
        if self.use_lang_id:
    ...
    def forward(
        ...
        if view or return_plot_as_filepath:
            ...
            plt.subplots_adjust(
                left=0.05, bottom=0.12, right=0.95, top=0.9, wspace=0.0, hspace=0.0
            )
    ...
    def read_aloud(
        ...
        if not blocking:
            ...
            sounddevice.play(
                torch.cat((wav, torch.zeros([6000])), 0).numpy(), samplerate=24000
            )
            sounddevice.wait()
