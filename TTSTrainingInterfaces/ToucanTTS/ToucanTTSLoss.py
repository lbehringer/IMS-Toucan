...
class ToucanTTSLoss(torch.nn.Module):
    ...
    def forward(self, predicted_features, gold_features, features_lengths, text_lengths, gold_durations, predicted_durations, predicted_pitch, predicted_energy, gold_pitch, gold_energy):
        ...
        # calculate loss
        gold_indexes_by_codebook = torch.stack(torch.split(gold_features, split_size_or_sections=8, dim=-1))
        ce = list()
        for one_hot_pred, one_hot_target in zip(predicted_features, gold_indexes_by_codebook.transpose(2, 3)):
        ...
        # apply weight
        distance_loss = distance_loss.mul(out_weights.transpose(1, 2)).masked_select(out_masks.transpose(1, 2)).sum()
