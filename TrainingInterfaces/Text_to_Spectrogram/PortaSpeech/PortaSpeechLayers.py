...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        if norm_type == "bn":
            ...
            norm_builder = lambda: SequentialWrappableConditionalLayerNorm(
                normal_shape=1, speaker_embedding_dim=spk_emb_size
            )
        else:
...
class ConditionalConvBlocks(ConvBlocks):
    ...
    def forward(self, x, cond, nonpadding=None, utt_emb=None):
        ...
        x = x * nonpadding
        x = super(ConditionalConvBlocks, self).forward(
            x, utt_emb=utt_emb
        )  # input needs to be BTC
        if self.is_BTC_:
            x = x.transpose(1, 2)
        return x
